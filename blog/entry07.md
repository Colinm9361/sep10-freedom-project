# Entry 7
##### 6/7/26

# Context:

## Content
Throughout my time making my freedom project, the topic I chose was agriculture. something that really stuck to me was farming, and it always interest me. So my freedom project looked at the positives, the hardware, software and possible future solutions use or plan to use. As I worked throughout and until now, the aspect of looking into topics that are more agricultural or environmental always stuck to me.  

## Tool
The tool I used for my freedom project was the HTML code, Aframe. A 3D modeling website using HTML code that allows coders such as myself to create 3D models using shapes to create whatever I like. Aframe is a 3D and VR engine where you can use HTML to create games, block images, and scenery. I'm going to use it to help build something based on the Freedom project. An ai powered tractor, or truck.

## In Class Presentation
Upon completing my project on agriculture, Every student went one by one to present their projects in class where they would teach students briefly on the topic, the tool they used to justify means, and throughout the presentations, I was one of them. Some takeaways from the in class presentation I got was to be formal, make a lot of eye contact, make sure I have everything in place, and be active/entertaining with your crowd. I had a lot of fun with it, and really enjoyed sharing my topic on agriculture.

[Slides](https://docs.google.com/presentation/d/1LzcmvrWL_e63xZY-kVT_IjnLQiKZSwlWidQe3tk1myU/edit?slide=id.p#slide=id.p)

## Convention
After the in class presentation, a couple of days later we had to present our projects to juniors and seniors where I presented my project with an elevator pitch. From there I didn't particularly learn much from the in class presentation except, get to the point, and focus on the more important points. With this help, I had a lot of fun and enjoyed the convention where I talked and saw other students projects too, where we discussed how it went. 

[Project](https://colinm9361.github.io/sep10-freedom-project/)

# Conclusion:
So to sum up, throughout my time working on this freedom project and going to the expo and sharing my project with others, I had a lot of fun and learnt a ton from my time in the project.

[Previous](entry06.md) | [Next](entry08.md)

[Home](../README.md)
